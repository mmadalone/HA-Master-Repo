...
15. **Never create a blueprint or script without a header image** in its description, **and never leave a broken image reference** (file referenced but not on disk) *(project convention — not an HA community standard, but mandatory for this project)*. This is a **blocking gate** — do not write a single line of YAML until the header image is either approved by the user or explicitly declined. Always ask. If the user blows past the question, insist — repeat the ask and do not proceed until you get a clear answer. **During reviews**, verify the referenced image file actually exists at `HEADER_IMG` (`GIT_REPO/images/header/` — see Project Instructions for resolved path). Allowed formats: `.jpeg`, `.jpg`, `.png`, `.webp`. See §11.1 step 4 for default image specs (1K, 16:9, Rick & Morty style).
