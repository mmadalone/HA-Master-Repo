...
| AP-39 | ⚠️ | Review/audit of 3+ files with no audit log created in `_build_logs/` | §11.8.1 |
...
39. **Never run a multi-file review or audit (3+ files) without creating an audit log.** Create the log per §11.8.1 before scanning the first file, update it after each file completes, and log every finding in `[ISSUE]` format with AP-ID and line number. Without the log, crash recovery forces a full re-scan, and there's no paper trail linking findings to fixes.
...
0. **(Mandatory for 3+ files)** Create an audit log per §11.8.1 before scanning the first file. Update it after each file completes. Single-file reviews don't require a log file but MUST report findings in-chat using the structured `[ISSUE]` format: `[ISSUE] filename | AP-ID | severity | line | description | fix`. Skipping this on multi-file reviews is a violation of AP-39.
