...
**Style Guide Version: 2.1 — 2026-02-11** · Bump this on structural changes (new files, section renumbering, directive additions).

> **What you are reading:** This is a structured style guide for AI-assisted Home Assistant development. It governs how you generate YAML, prompts, and configs for this user's HA instance. The guide is split across 9 files (~74K tokens total — but you should never load more than ~15K for any task). **Do not load all files for every task** — use the routing table below to load only what's needed.
...
**Always load:** **§1 Core Philosophy only** (~5.3K tokens). Also load §2.3 (pre-flight checklist) before any file edit. Load the full file (including rest of §2, §9, §12) only for complex/multi-domain tasks.
...
|-----|----------|---------|--------|
| [Core Philosophy](00_core_philosophy.md) | §1, §2, §9, §12 | ~8.2K (§1 alone: ~5.3K) | Design principles, versioning workflow, naming conventions, communication style |
| [Blueprint Patterns](01_blueprint_patterns.md) | §3, §4 | ~6.4K | Blueprint YAML structure, inputs, variables, templates, script standards |
| [Automation Patterns](02_automation_patterns.md) | §5 | ~5.9K | Error handling, modes, timeouts, triggers, GPS bounce, helpers, area/label targeting |
| [Conversation Agents](03_conversation_agents.md) | §8 | ~7.8K | Agent prompt structure, separation from blueprints, naming conventions |
| [ESPHome Patterns](04_esphome_patterns.md) | §6 | ~5.6K | Device config structure, packages, secrets, wake words, naming |
| [Music Assistant Patterns](05_music_assistant_patterns.md) | §7 | ~11.0K | MA players, play_media, TTS duck/restore, volume sync, voice bridges |
| [Anti-Patterns & Workflow](06_anti_patterns_and_workflow.md) | §10, §11 | ~9.4K (scan table: ~3.5K) | Things to never do, build/review/edit workflows |
| [Troubleshooting & Debugging](07_troubleshooting.md) | §13 | ~5.7K | Traces, Developer Tools, failure modes, log analysis, domain-specific debugging |
| [Voice Assistant Pattern](08_voice_assistant_pattern.md) | §14 | ~11.2K | End-to-end voice stack architecture: ESPHome satellites, pipelines, agents, blueprints, tool scripts, helpers, TTS |
...
**14 top-level sections · 119 subsections · 39 anti-patterns (35 AP codes + 4 sub-items) · 8 security checks · 9 files**
