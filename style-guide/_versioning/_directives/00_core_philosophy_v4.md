...
This style guide is ~15K tokens across 9 files. **Never load all files for every task.** AI context is expensive — every token spent on irrelevant rules is a token not available for the user's actual content.

**Priority tiers — what to load and when:**

| Tier | When to load | Files | ~Tokens |
|------|-------------|-------|---------|
| **T0 — Always** | Every task, no exceptions | `00_core_philosophy.md` (§1 only, skip §2/§9/§12 unless editing those) | ~2K |
| **T1 — Task-specific** | When the routing table (master index) maps to it | The ONE pattern doc for the task at hand | ~1.5-2.5K |
| **T2 — Review/edit** | Only when reviewing or editing existing code | `06_anti_patterns_and_workflow.md` (§10 scan table + relevant §11 workflow) | ~2K |
| **T3 — Reference only** | Only when explicitly needed | `07_troubleshooting.md`, `08_voice_assistant_pattern.md`, `09_naming` (in §9) | ~1-2K each |
