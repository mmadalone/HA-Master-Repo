...
Every distinct step or phase in the action sequence must have BOTH:

1. **A numbered comment** explaining what the step does and why.
2. **An `alias:` field** on the action itself — this is what shows up in HA's trace UI.

```yaml
action:
  # 1) Wait for entrance sensor — don't proceed until someone is physically
  #    at the door (GPS arrival alone isn't enough)
  - alias: "Wait for entrance sensor detection"
    wait_for_trigger:
      - platform: state
  ...
  # 2) Check if entrance wait timed out — clean up and bail if nobody showed
  - alias: "Handle entrance timeout"
    if:
      - condition: template
  ...
  # 3) Reset speakers — power-cycle to clear stale Bluetooth connections
  - alias: "Reset speaker switches"
    action: switch.turn_off
    target: !input speaker_switches
```

**Why both?** Comments explain the *reasoning* for developers reading the YAML. `alias` labels make traces readable for debugging in the HA UI. Every `choose` branch, every `if/then`, every service call gets an `alias`.
...
Script sequences must include comments AND `alias` fields on each step, the same way as blueprint actions (see §3.5). If a script has multiple steps, each gets a numbered comment with an explanation and an `alias` for trace readability.
