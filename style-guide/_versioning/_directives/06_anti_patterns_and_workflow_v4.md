...
## Issues Found
<!-- One line per issue. Severity uses §1.11 taxonomy. -->
| # | File | Severity | Section | Description | Suggested fix | Status |
|---|------|----------|---------|-------------|---------------|--------|
| 1 | bedtime.yaml | ❌ ERROR | actions | `wait_for_trigger` missing timeout | Add timeout: '00:05:00' + handle wait.completed | OPEN |
| 2 | bedtime.yaml | ⚠️ WARNING | inputs | Hardcoded entity_id in action block | Move to !input with default | OPEN |
| 3 | bedtime.yaml | ℹ️ INFO | header | Missing description image | Generate header image per §3.1 | FIXED |
...
```
[ISSUE] bedtime.yaml | ❌ ERROR | actions | wait_for_trigger missing timeout | add timeout + handle wait.completed
[ISSUE] bedtime.yaml | ⚠️ WARNING | inputs | hardcoded entity_id | move to !input
[FIXED] bedtime.yaml | added header image (was ℹ️ INFO issue)
```
