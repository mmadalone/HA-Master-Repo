...
This style guide is ~58K tokens across 9 files (plus master index). **Never load all files for every task.** AI context is expensive — every token spent on irrelevant rules is a token not available for the user's actual content.

**Priority tiers — what to load and when:**

| Tier | When to load | Files | ~Tokens (measured) |
|------|-------------|-------|--------------------|
| **T0 — Always** | Every task, no exceptions | `00_core_philosophy.md` (§1 only, skip §2/§9/§12 unless editing those) | ~3.5K (§1 alone) |
| **T1 — Task-specific** | When the routing table (master index) maps to it | The ONE pattern doc for the task at hand | 3–5K (see table below) |
| **T2 — Review/edit** | Only when reviewing or editing existing code | `06_anti_patterns_and_workflow.md` (§10 scan table + relevant §11 workflow) | ~8.3K full, ~3.5K scan table only |
| **T3 — Reference only** | Only when explicitly needed | `07_troubleshooting.md`, `08_voice_assistant_pattern.md` | ~5K / ~8.4K |
...
|------|-----------|-----------------------------------------|
| Master index | ~3.2K | ~1.2K (routing table only) |
| `00_core_philosophy.md` | ~7.3K | ~3.5K (§1 only, drop §2/§9/§12) |
| `01_blueprint_patterns.md` | ~4.8K | ~4.8K (usually need all of it) |
| `02_automation_patterns.md` | ~4.7K | ~2.5K (§5.1 + §5.4 for most tasks) |
| `03_conversation_agents.md` | ~4.7K | ~3K (§8.1–8.4 for most tasks) |
| `04_esphome_patterns.md` | ~4.1K | ~4.1K (load fully for ESPHome tasks) |
| `05_music_assistant_patterns.md` | ~7.6K | ~4K (duck/restore + play_media + voice bridge sections) |
| `06_anti_patterns_and_workflow.md` | ~8.3K | ~3.5K (scan table + one workflow section) |
| `07_troubleshooting.md` | ~5.0K | ~2K (load specific §13.X on demand) |
| `08_voice_assistant_pattern.md` | ~8.4K | ~3.5K (relevant layers only) |
| **Total if everything loaded** | **~58K** | **Never do this** |
...
| 2 | **Versioning** (§2.3 pre-flight) | Unversioned edits are irreversible — version before everything else |
...
- **Versioning** (#2) — pre-flight backup before destructive edits
