...
├── blueprints/     ← blueprint YAML versions
...
| Blueprints (`.yaml`) | `blueprints/` | `coming_home_v1.yaml` |
...
1. **Copy the current file** to the appropriate `_versioning/` subdir with the current version number:
   - Editing `coming_home.yaml`? → copy it as `_versioning/blueprints/coming_home_v1.yaml`
...
2. **Add or update the changelog** in the same subdir, named after the base file:
   - `_versioning/blueprints/coming_home_changelog.md`
