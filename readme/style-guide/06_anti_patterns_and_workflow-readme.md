...
At ~20.4K tokens, this is never loaded in full. The scan tables (~4.9K) plus the relevant workflow section is the typical load.
...
**Audit resilience** (§11.15) — deep-pass audits split into four stages, each loading only its required style guide sections. Per-check granularity (§11.15.2) logs each check result immediately, so crashes lose minimal work. Pre-flight estimation (§11.15.3) sizes stages against context capacity before starting. Stage splitting (§11.15.4) decomposes oversized stages into sub-stages. Decision reasoning capture and interpretation notes survive crashes so recovery sessions don't re-litigate settled questions.
