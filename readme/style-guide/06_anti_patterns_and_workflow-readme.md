...
**AP-39 — the build log gate** — every BUILD-mode file edit requires a build log in `_build_logs/` before the first write. Every AUDIT check command requires a log pair (progress + report). Zero threshold — even a one-line fix gets logged. The companion **log-after-work invariant** requires updating the log after every edit or check completion before proceeding. Together with the log-before-work invariant, the sequence is: log → work → update log → next work. This is what makes crash recovery possible.
