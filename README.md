...
All code in this repository is generated by **Claude 4.6/4.7** (Anthropic). All images are generated by **Google Gemini**.
...
- 2× Home Assistant Voice Preview Edition (workshop + living room)
- 2× Aqara FP2 presence sensors (multi-zone: workshop/entrance/kitchen, living room/bedroom, bathroom shower/sink/toilet)
- Aqara G3 camera hub
...
- [OpenAI](https://openai.com/) (GPT-4o) — LLM backend for conversation agents
- [ElevenLabs](https://elevenlabs.io/) — text-to-speech with custom character voices
- [Music Assistant](https://music-assistant.io/) — multi-room audio management (when Spotify's rate limiter isn't having a damn seizure)
...
The "Hey" wake words route to the standard agent — fast, short answers for things like "Hey Rick, what time is it?" The "Yo" wake words route to the verbose agent variant — longer, more detailed responses with expanded tool access for when you actually want a conversation instead of just barking orders like some Cardassian overseer.
...
### 📡 Volume Ducking
Automatic volume management for TTS announcements — saves current volume state, ducks media, plays the announcement, restores volume. Handles the race condition between TTS completion and volume restore.

---

## Style Guide — The Rules of Acquisition

The `/HA Master Style Guide/` directory contains the **Rules of Acquisition** — a comprehensive YAML generation style guide used by AI agents (primarily Claude) when generating Home Assistant configurations. Named after the Ferengi code of conduct from *Star Trek: Deep Space Nine*, the guide enforces opinionated standards with the same ruthless consistency Quark applies to bar tabs. Rick would call it "bureaucratic nonsense that happens to prevent mass YAML casualties."

The style guide isn't just a reference document — it's an active enforcement system with anti-pattern scanning, severity classifications, mandatory pre-flight checklists, and crash recovery protocols. Every AI session that touches HA configuration loads the relevant sections before writing a single line of YAML.

| File | Contents |
|------|----------|
| `ha_style_guide_project_instructions.md` | **Master index** — routes to all sections, defines operational modes (BUILD/TROUBLESHOOT/AUDIT), project paths, tool routing tables, and crash recovery protocols. This is the file loaded into Claude's project instructions field. |
...
The guide is opinionated — it enforces `!secret` usage in all examples, requires action aliases for trace debugging, mandates error handling on network calls, and includes AI-readability checks specifically designed for vibe coding workflows.

---

## Claude Project Configuration

This project is built inside a **Claude Project** — Anthropic's feature for persistent AI workspaces with custom instructions, file uploads, and cross-session memory. The project configuration is what turns a general-purpose AI into a Home Assistant specialist that knows the codebase, follows the style guide, and doesn't lose context between sessions. Think of it as programming the programmer — or, as Quark would put it, training the employee before they touch the merchandise.

### Project Instructions Field

The Claude Project's custom instructions field is the operational backbone — loaded into every conversation before Claude sees a single message. This is separate from the style guide itself; the instructions field tells Claude *how to work*, while the style guide tells Claude *what to write*.

The instructions field contains:

**Canonical Project Paths** — seven hardcoded paths that anchor every file operation: `PROJECT_DIR` (style guide source), `HA_CONFIG` (SMB-mounted Home Assistant config), `GIT_REPO` (publish-only mirror), `HEADER_IMG` and `HEADER_IMG_RAW` (blueprint header images, local and GitHub raw URLs), and `README_AUTO_DIR` / `README_SCRI_DIR` / `README_TEMPL_DIR` (per-type readme directories). If a path changes, it's updated once here and propagates everywhere.

**File Transfer Rules** — five non-negotiable rules for writing files to Home Assistant: never chunk over SSH, never base64 encode over SSH, use Desktop Commander `write_file` directly to the SMB mount, use append mode for files over 30KB, and always verify after writing. No exceptions. Rick didn't write these rules, but he'd respect the paranoia.

**Dual-Filesystem Architecture** — Claude operates across two distinct filesystems that must never be confused. Confusing them is the kind of mistake that makes Rick question whether you deserve electricity:
- **User's macOS** — accessed via Desktop Commander and Filesystem MCP tools. This is where all real work happens: HA config files (via SMB mount), style guide documents, build logs, and the git repo.
- **Claude's container** — an ephemeral Linux sandbox (`/home/claude/`) used only for temporary computation, artifact generation, and scratch work. Files here reset between sessions.
...
**Git Workflow** — the style guide and blueprints live in `PROJECT_DIR` and `HA_CONFIG` respectively. The git repo (`GIT_REPO`) is a publish-only mirror — never edit files there directly. Publishing follows a strict 3-phase chain: rsync source to repo, stage and commit via git MCP with descriptive messages, then wait for explicit user confirmation before pushing. Claude never auto-pushes. Rule of Acquisition #2: the best deal is the one where both parties walk away satisfied, and surprise pushes satisfy nobody.

**Helper File Routing** — input helpers are split by type across six YAML files (`helpers_input_boolean.yaml`, `helpers_input_button.yaml`, etc.) in `HA_CONFIG`. Before creating a new helper, check the relevant file — duplicates are the kind of sloppy overhead that would make Quark dock your pay.

**Context Management Rules** — nine rules to keep conversations efficient and prevent context window bloat: never paste full config files (reference by path and line range), produce state checkpoints after major tasks, write finalized configs to files immediately, one automation per session, proactive summarization after ~15 exchanges, front-load HA conventions, disable unused tools per session, no redundant code blocks, and bias toward artifact edits over conversational explanations.

**The Salvage Directive (Crash Recovery)** — when a session crashes mid-build (and they do), the instructions define a mandatory recovery protocol. On any crash-recovery trigger phrase, Claude must: acknowledge the crash, scan `_build_logs/` for incomplete entries, check git state via `ha_git_pending` and `ha_git_diff`, search past conversations for context, present a recovery summary, and wait for user confirmation before touching anything. The 23rd Rule of Acquisition applies: "Nothing is more important than your health — except your money." Time is money. Recover first.

**User Preferences** — the instructions field also carries persona directives for Claude itself. Currently: talk like Quark from DS9, cursing encouraged. This is why the build logs occasionally reference latinum.
...
- **BUILD** — full compliance checking, pre-flight checklists, mandatory build logs, git versioning. Used for any file creation or modification.
- **TROUBLESHOOT** — debugging focus, minimal ceremony, trace analysis tools. Used when something's broken.
- **AUDIT** — systematic violation scanning with anti-pattern tables, severity classifications, audit logging. Used for code review passes.

### Style Guide Section Loading

Claude doesn't load the entire style guide every session — that would be wasteful, and waste is anathema to profit. The instructions specify: always load §1 Core Philosophy (~5K tokens) plus §2.3 (pre-flight checklist) before any file edit. Load the full index (including §2, §9, §12) only for complex or multi-domain tasks. Specific sections load on demand based on the operational mode and task type.

---

## Development Toolchain

The vibe coding workflow depends on a specific set of MCP (Model Context Protocol) tools that give Claude direct access to the user's filesystem, Home Assistant instance, and git repositories. These aren't optional — they're what make AI-assisted HA development actually work instead of devolving into some hoo-mon typing YAML by hand like an animal.
...
| **Filesystem MCP** | Secondary file access — directory listings, file info, search, and bulk reads across the user's filesystem. |
| **Home Assistant MCP** | Direct integration with the HA instance — automation management, entity state queries, service calls, configuration validation, git checkpoints, and add-on management. |
| **Git MCP** | Repository operations with structured responses — status, diff, staging, commits, branching. Eliminates terminal output parsing. |
...
- [Home Assistant community blueprints exchange](https://community.home-assistant.io/c/blueprints-exchange/53) — various patterns and approaches. Some of it's worth stealing. Legally.
...
If you recognize your work here and I haven't credited you, please reach out and I'll fix it immediately. Quark believes in paying debts — selectively — but credit where it's due is free, and free is Quark's second favorite price after "whatever they'll pay."
